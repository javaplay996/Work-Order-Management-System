    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑工单沟通</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">工单沟通管理</li>
                        <li class="breadcrumb-item active">编辑工单沟通</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">工单沟通信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="gongdanId" name="gongdanId" type="hidden">
                                    <div class="form-group  col-md-6">
                                        <label>沟通内容</label>
                                        <input id="gongdanGoutongContentupload" name="file" type="file">
                                        <script id="gongdanGoutongContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var gongdanGoutongContentUe = UE.getEditor('gongdanGoutongContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="gongdanGoutongContent" id="gongdanGoutongContent-input">
                                    </div>
                                <input id="yuangongId" name="yuangongId" type="hidden">
                                    <div class="form-group  col-md-6">
                                        <label>回复内容</label>
                                        <input id="gongdanHuifuContentupload" name="file" type="file">
                                        <script id="gongdanHuifuContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var gongdanHuifuContentUe = UE.getEditor('gongdanHuifuContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="gongdanHuifuContent" id="gongdanHuifuContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#gongdanGoutongContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('gongdanGoutongContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });


        $('#gongdanHuifuContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('gongdanHuifuContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            if(window.sessionStorage.getItem('role') != '员工' && window.sessionStorage.getItem('role') != '用户'){//当前登录用户不为这个
                if($("#yuangongId") !=null){
            ...
            httpJson("gongdanGoutong/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addgongdanGoutong');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.gongdanGoutongContent != null && ruleForm.gongdanGoutongContent != 'null' && ruleForm.gongdanGoutongContent != '' && $("#gongdanGoutongContentupload").length>0) {

            var gongdanGoutongContentUeditor = UE.getEditor('gongdanGoutongContentEditor');
            gongdanGoutongContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.gongdanGoutongContent != null){
                    mes = ''+ ruleForm.gongdanGoutongContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                gongdanGoutongContentUeditor.setContent(mes);
            });
        }
        if (ruleForm.gongdanHuifuContent != null && ruleForm.gongdanHuifuContent != 'null' && ruleForm.gongdanHuifuContent != '' && $("#gongdanHuifuContentupload").length>0) {

            var gongdanHuifuContentUeditor = UE.getEditor('gongdanHuifuContentEditor');
            gongdanHuifuContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.gongdanHuifuContent != null){
                    mes = ''+ ruleForm.gongdanHuifuContent;
                    mes = mes.replace(/\n/g, "<br>");
                }
                gongdanHuifuContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#gongdanGoutongContentupload").length>0) {
            var gongdanGoutongContentEditor = UE.getEditor('gongdanGoutongContentEditor');
            if (gongdanGoutongContentEditor.hasContents()) {
                $('#gongdanGoutongContent-input').attr('value', gongdanGoutongContentEditor.getPlainTxt());
            }
        }
        if($("#gongdanHuifuContentupload").length>0) {
            var gongdanHuifuContentEditor = UE.getEditor('gongdanHuifuContentEditor');
            if (gongdanHuifuContentEditor.hasContents()) {
                $('#gongdanHuifuContent-input').attr('value', gongdanHuifuContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                gongdanId: "required",
                gongdanGoutongContent: "required",
                insertTime: "required",
                yuangongId: "required",
                gongdanHuifuContent: "required",
                updateTime: "required",
            },
            messages: {
                gongdanId: "工单不能为空",
                gongdanGoutongContent: "沟通内容不能为空",
                insertTime: "发起时间不能为空",
                yuangongId: "回复人不能为空",
                gongdanHuifuContent: "回复内容不能为空",
                updateTime: "回复时间不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addgongdanGoutong = window.sessionStorage.getItem("addgongdanGoutong");
        if (addgongdanGoutong != null && addgongdanGoutong != "" && addgongdanGoutong != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("gongdanGoutong/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#gongdanGoutongContent").val(ruleForm.gongdanGoutongContent);
        $("#insertTime-input").val(ruleForm.insertTime);
        $("#yuangongId").val(ruleForm.yuangongId);
        $("#gongdanHuifuContent").val(ruleForm.gongdanHuifuContent);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            var gongdanHuifuContentEditor = UE.getEditor('gongdanHuifuContentEditor');
            gongdanHuifuContentEditor.ready(function () {
                gongdanHuifuContentEditor.setDisabled('fullscreen');
            });
        }
            ...
            $(".yuangong").remove();//删除当前用户的信息
            var gongdanGoutongContentEditor = UE.getEditor('gongdanGoutongContentEditor');
            gongdanGoutongContentEditor.ready(function () {
                gongdanGoutongContentEditor.setDisabled('fullscreen');
            });
        }
        else{
            // alert("未知情况.......");
