        ...
        <div id="content">
            ...
            <div class="container mt-0">
                <div class="row breadcrumb-bar">
                    <div class="col-md-6">
                        <h3 class="block-title">工单沟通管理</h3>
                    </div>
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">工单沟通列表</li>
                        </ol>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="widget-area-2 lochana-box-shadow">
                            <h3 class="widget-title">工单沟通列表</h3>
    ...
    <script>
        ...
        function setDataRow(item, number) {
            ...
            row.appendChild(yuangongPhotoCell);
                ...
                //沟通内容
            var gongdanGoutongContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.gongdanGoutongContent && item.gongdanGoutongContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.gongdanGoutongContent.indexOf('<img');
                var endIndex = item.gongdanGoutongContent.indexOf('>');
                    gongdanGoutongContentCell.innerHTML = item.gongdanGoutongContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.gongdanGoutongContent != null && item.gongdanGoutongContent != "" && item.gongdanGoutongContent.length >= 11) {
                        gongdanGoutongContentCell.innerHTML = item.gongdanGoutongContent.substring(0, 10) + "...";
                } else {
                        gongdanGoutongContentCell.innerHTML = item.gongdanGoutongContent;
                }
            }
            row.appendChild(gongdanGoutongContentCell);
            ...
            row.appendChild(insertTimeCell);
                ...
                //回复内容
            var gongdanHuifuContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.gongdanHuifuContent && item.gongdanHuifuContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.gongdanHuifuContent.indexOf('<img');
                var endIndex = item.gongdanHuifuContent.indexOf('>');
                    gongdanHuifuContentCell.innerHTML = item.gongdanHuifuContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.gongdanHuifuContent != null && item.gongdanHuifuContent != "" && item.gongdanHuifuContent.length >= 11) {
                        gongdanHuifuContentCell.innerHTML = item.gongdanHuifuContent.substring(0, 10) + "...";
                } else {
                        gongdanHuifuContentCell.innerHTML = item.gongdanHuifuContent;
                }
            }
            row.appendChild(gongdanHuifuContentCell);
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("gongdanGoutong/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addgongdanGoutong", "addgongdanGoutong");
            window.location.href = "add-or-update.jsp"
        }
